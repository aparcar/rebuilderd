#!/bin/bash
# rebuilderd backend for OpenWrt targets, rebuilt from source: the firmware
# images plus the target packages and kmods the same build produces.
#
...
#   $2                 URL $1 was fetched from, under .../targets/<target>/<subtarget>/
#                      — release, target and subtarget are parsed from its path
...
INPUT_URL="${2:?missing URL argument — need a .../targets/<target>/<subtarget>/ URL}"

# --- derive release / target / subtarget from the input URL ---
url="${INPUT_URL%%[?#]*}"
case "$url" in
    */targets/*) ;;
    *) echo "no /targets/ segment in input URL: $url" >&2; exit 1 ;;
esac
after="${url#*/targets/}"
TARGET="${after%%/*}"
after="${after#*/}"
...
case "$url" in
    ...
    *) echo "cannot derive release from input URL: $url" >&2; exit 1 ;;
esac
...
BUILD_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/rebuilderd-openwrt.XXXXXX")
