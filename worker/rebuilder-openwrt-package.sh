...
#   $2                 URL $1 was fetched from (the .apk URL)
...
INPUT_URL="${2:?missing URL argument — need the .apk URL to derive arch/release}"

# --- derive arch / release from the .apk URL ---
url="${INPUT_URL%%[?#]*}"
case "$url" in
    */packages/*) ;;
    *) echo "no /packages/ segment in input URL: $url" >&2; exit 1 ;;
esac
after="${url#*/packages/}"
ARCH="${after%%/*}"
case "$url" in
